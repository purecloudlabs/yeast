...
function calculateSpaceDiff(oldString: string, newString: string, oldIdx: number, newIdx: number, oldWords: string[], newWords: string[]): SpaceDiffData {
    // init
    let oldSpace: number;
    let newSpace: number;
...
function addSpaceMod(s: string, boundaries: WordBoundaryData, idx: number, mods: ModificationData[], diffSource: DiffSource, diffType: DiffType) {
    // init
...
function addMod(s: string, words: string[], wordPos: number, mods: ModificationData[], boundaries: WordBoundaryData, modSubtype: DiffType, diffSource: DiffSource, spaceDiff?: number, initSpaceDiff?: number) {
    // init
    ...
    while (oldIdx < oldWords.length && newIdx < newWords.length) {
        ...
        if (oldWords[oldIdx] !== newWords[newIdx]) {
            ...
            const {
                ...
                oldMatchIdx,
                newMatchIdx
            }: DiffData = getDiffData(oldIdx, newIdx, oldWords, newWords);
 ...
 */ 
function calculateDiffScores(testType: DiffType, oldIdx: number, newIdx: number, oldEntities: DiffEntity[], newEntities: DiffEntity[]): ScoreData {
    // init
    let score: number = 0;
...
function getDiffData(oldIdx: number, newIdx: number, oldEntities: DiffEntity[], newEntities: DiffEntity[]): DiffData {
    ...
    const tiedScores: DiffTypeScoreMap[] = descendingScores.filter((score: DiffTypeScoreMap) =>  score[Object.keys(score)[0]] === maxScore[Object.keys(maxScore)[0]]); // Object.values(score)[0] === Object.values(maxScore)[0]);
...
function diffInner(oldNodes?: YeastChild[], newNodes?: YeastChild[]): YeastChild[] {
    // init
    ...
    if (areOldNodesEmpty) {
        diffNodes = newNodes.map((node: YeastNode) => {
            ...
            const diffNode: YeastNode = Object.assign({}, node);
    ...
    while (oldIdx < oldNodes.length || newIdx < newNodes.length) {
        ...
        let diffChildren: YeastChild[] = [];
        diffChildren = diffInner(oldNode?.children || [], newNode?.children || []);
                ...
                if (diffData.oldMatchIdx) {
                    for (let i = oldIdx + 1; i <= diffData.oldMatchIdx; i++) {
                        ...
                        if (i === diffData.oldMatchIdx) {
                            removedNode = Object.assign({}, oldNodes[i]) as YeastNode;
            ...
            // Otherwise the diff type is Modified.
                ...
                // If the node is modified but no text within it is modified, no need to get text mod data.
                    ...
                    if (containsTextModification(diffChildren) && oldNode.type === newNode.type) {
                        ...
                        const oldDiffNode: YeastNode = Object.assign({}, oldNode);
...
function isNodeMatch(oldNode: YeastNode, newNode: YeastNode, diffChildren: YeastChild[]): EntityMatchData {
    ...
    const areBothContentGroupItem: boolean = oldNode.type === YeastBlockNodeTypes.ContentGroupItem && newNode.type === YeastBlockNodeTypes.ContentGroupItem;
    if (areBothContentGroupItem) {
        ...
        const oneTitleExists: boolean = (!oldContentGroupItem.title && !!newContentGroupItem.title) || (!!oldContentGroupItem.title && !newContentGroupItem.title);
        let doTitlesMatch: boolean = true;
...
export function diff(oldNode: DocumentNode | undefined, newNode: DocumentNode | undefined): DocumentNode | undefined {
    ...
    if (oldNode === undefined) {
        const diffNode: DocumentNode = Object.assign({}, newNode);
        diffNode.hasDiff = true;
        diffNode.diffType = DiffType.Added;
        return diffNode;
    }
    if (newNode === undefined) {
        const diffNode: DocumentNode = Object.assign({}, oldNode);
        diffNode.hasDiff = true;
        diffNode.diffType = DiffType.Removed;
        return diffNode;
    }
    ...
    const diffChildren: YeastChild[] = diffInner(oldNode.children, newNode.children);
