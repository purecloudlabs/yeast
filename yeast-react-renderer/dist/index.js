...
import React, { useState as useState$3, useRef as useRef$6, useEffect as useEffect$5 } from 'react';
...
function ImageNodeRenderer(props) {
    ...
    const [newSrc, setNewSrc] = useState$3();
    const [oldAlt, setOldAlt] = useState$3();
        ...
        try {
            if (match && !isSameHost) {
                ...
                if (parentMatch) {
                    ...
                    const filename = changesetPathMatch[1];
    ...
    const resolveParentPath = (src, assetKeyPath, parentMatch) => {
        if (!parentMatch || !assetKeyPath.includes('/'))
            return src;
